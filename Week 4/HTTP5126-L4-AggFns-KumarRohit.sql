...
-- B
SELECT COUNT(id),category AS Mammal FROM stock_items GROUP BY  category HAVING category!="Piscine";
-- C
...
-- 4
SELECT item AS "Product",CONCAT("$",price) AS "Price",inventory AS "Stock Remaining",category AS "Species",(price*inventory) AS "Potential earning" FROM stock_items ORDER BY "Potential earning" DESC;